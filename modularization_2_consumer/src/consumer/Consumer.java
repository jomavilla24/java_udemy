 ...
 * 
 * 1. In the project folder name modularization_2_geometric_shapes we have created
 * two packages:
 * - shapes:
 ...
 * modularization_2_geometric_consumer that will be using the utility in areUtil package
 * and the shapes class in the shapes package. 
 * 
 *  we will try to do it by changing the .classpath file inside the 
 * modularization_1_geometric_consumer project. Right click on modularization_2_geometric_consumer 
 * -> Properties; search the option "Java Build Path" on the left, the on the projects
 * tab select the .classmodule file and click on add, select the project modularization_2_geometric_shapes.
 * 
 * */
